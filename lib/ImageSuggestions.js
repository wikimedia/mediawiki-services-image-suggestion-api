...
const MediaSearchResults = require('./MediaSearchResults');
...
class ImageSuggestions {
	 ...
	 */
	validateParams(pathParams, query, defaultSeed) {
	 ...
	 */
	getPseudoRandomRowNums(pageTableRowCount, seed, limit, offset) {
	 ...
	 */
	async getRowNums(algoResults, wikiId, id, seed, limit, offset, source) {
		let rowNums = [];
		if (id.length > 0) {
			rowNums = await algoResults.queryDBForRowNums(wikiId, source, id);
		} else if (seed > 0) {
			rowNums = this.getPseudoRandomRowNums(
	 ...
	 */
	makeResponse(id, seed, pages, noFilter) {
	...
	async getPages(req) {
		...
		const {
			...
			offset,
			wikiId
		} = this.validateParams(req.params, req.query, defaultSeed);
		const seed = this.getSeed(req.query, defaultSeed);

		const database = new Database(req.logger);
		const algoResults = new AlgoResults(database);
		const mediaSearchResults = new MediaSearchResults(req.logger);
		const rowNums = await this.getRowNums(
			algoResults,
			wikiId,
			id,
			seed,
		...
		if (req.query.source && req.query.source !== 'ms') {
			return this.makeResponse(id, seed, pages, req.query.noFilter);
		}
...
module.exports = ImageSuggestions;
