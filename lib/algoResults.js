...
class AlgoResults {
	...
	/**
	 * Returns the size of the specified page table
	 *
	 * @param {string} id wiki id of interest
	 * @return {integer} the table size
	 */
	getPageTableRowCount(id) {
		...
		return pageTableRowCount[`${id}_page`];
	}
	...
	async queryDBForPages(id, query, rowNums = []) {
		...
		if (rowNums.length === limit) {
			whereTest = 'outerPage.row_num IN (' + rowNums.join() + ')';
		}
	...
	populateDatabase(tsvDir) {
		return new Promise((resolve, reject) => {
			...
			filesToProcess.forEach((tsvFile) => {
				...
				lineReader.on('line', (line) => {
					if (lineCount === 0) {
						...
						imageRow.pop(); // wiki id is implied in table name, so don't store

						// We may get multiple consecutive rows for the same page.
						// Only add if this is a new page.
				...
				lineReader.on('close', () => {
					...
					pageTableRowCount[`${id}_page`] = pageRowNum - 1;
					completedFiles++;
					if (completedFiles === filesToProcess.length) {
						resolve();
					}
				});
			});
