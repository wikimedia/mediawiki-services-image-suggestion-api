...
function initAlgoResultsSync(dataPath = './static') {
    ...
    newFiles.forEach((file) => {
        tsvToJson.tsvFileToJsonFileSync(
            dataPath, file.tsvFile, file.jsonFile, false, (json, newObj) => {
                // needs to push the suggestions on even if json is empty (Which it would be)
                if (json.length === 0 || json[json.length - 1].page !== newObj.page_title) {
                    json.push({
                        project: newObj.wiki,
