...
class AlgoResults {
	 ...
	 */
	async queryDBForRowNumsById(wikiId, source, id) {
		const inStr = id.join();
	...
	/**
	 * Query database for rownums corresponding to the input page title
	 *
	 * @param {string} wikiId combination of wiki type and language (e.g. enwiki, arwiki)
	 * @param {string} source the source to consider (if any)
	 * @param {string} title page title in dbkey format
	 * @return {Promise} resolves as array of row nums, or rejected if error occurs.
	 */
	async queryDBForRowNumsByTitle(wikiId, source, title) {
		const colName = source === 'ima' ? 'row_num_ima' : 'row_num';
		const selectStatement =
			`SELECT page.${colName}
			 FROM ${wikiId}_page page
		...
		try {
			const results = await this.database.exec(selectStatement, [title]);
			return results.map((result) => {
				return result[colName];
			});
		} catch (err) {
			...
			throw new HTTPError({
				...
				type: 'error',
				title: 'Error',
				detail: `Unable to retrieve page id for ${title}`
			});
		}
	}
