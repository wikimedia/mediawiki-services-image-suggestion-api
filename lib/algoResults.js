...
class AlgoResults {
	...
	async initFromExistingDb(tsvDir) {
		// @todo: should we just iterate the recognized languages from wikiId instead?
		...
		for (const tsvFile of filesToProcess) {
			...
			const allSql = `SELECT MAX (row_num) AS max FROM ${id}_page`;
			const allResults = await this.database.exec(allSql);
			console.log(allResults);
			pageTableAllRowCount[`${id}_page`] = allResults[0].max;

			const imaSql = `SELECT MAX (row_num_ima) AS max FROM ${id}_page`;
			const imaResults = await this.database.exec(imaSql);
			console.log(imaResults);
			pageTableImaRowCount[`${id}_page`] = imaResults[0].max;
		}
	}
}
