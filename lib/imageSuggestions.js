...
function validateParams(pathParams, query, defaultSeed) {
	...
	if (id.length > 0) {
		if (title !== null) {
			throw new HTTPError({
				...
				type: 'bad_request',
				title: 'Bad Request',
				detail: 'cannot use title and id together'
			});
		}
	...
	if (title) {
		// Handle spaces vs. underscores. Don't handle first-letter capitalization
		// because that's complicated, locale-dependent and project-dependent, nor
		// Unicode normalization because it's unlikely the client would do it wrong.
		title = title.replace(/ /g, '_');
	}

	if (id.length > 0 || title !== null) {
		if (query.seed !== undefined || query.limit !== undefined || query.offset !== undefined) {
			throw new HTTPError({
				status: 400,
				type: 'bad_request',
				title: 'Bad Request',
				detail: 'If id or title is provided, then seed, limit, and offset cannot be provided'
			});
	...
	return { id, title, limit, offset, wikiId };
}
 ...
 */
 async function getRowNums(algoResults, wikiId, id, title, seed, limit, offset, source) {
	let rowNums = [];
	if (id.length > 0) {
		rowNums = await algoResults.queryDBForRowNumsById(wikiId, source, id);
	} else if (title !== null) {
		rowNums = await algoResults.queryDBForRowNumsByTitle(wikiId, source, title);
	} else if (seed > 0) {
...
async function getPages(req) {
	...
	const { id, title, limit, offset, wikiId } = validateParams(req.params, req.query, defaultSeed);
	...
	if (id.length === 0 && title === null || rowNums.length > 0) {
